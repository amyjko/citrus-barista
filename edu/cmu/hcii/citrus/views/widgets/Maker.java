...
public class Maker extends View {

	public static final Dec<Bool> clipsChildren = new Dec<Bool>(new Bool(false));

	public static final Dec<Namespace> typeToMake = new Dec<Namespace>(false, new Parameter<Namespace>());
	
	public static final Dec<View> label = new Dec<View>("(a Label text=typeToMake.name)", true);
	...
	public static final Dec<List<Behavior>> behaviors = new Dec<List<Behavior>>(new List<Behavior>(
		...
		new Behavior(App.mouse.leftButton.pressed, new Action() { public boolean evaluate(View t) {
			...
			if(t.get(elementToDrag) == null)
				t.set(elementToDrag, t.get(typeToMake).instantiate(null));
			...
			if(newView.reactsTo(App.mouse.pointer.dragged).value) {
				...
				debug("The view created for this new instance of " + t.get(typeToMake) + " doesn't react to dragging.");
				return false;
			}
	...
	public Maker(ArgumentList arguments) { super(arguments); }
